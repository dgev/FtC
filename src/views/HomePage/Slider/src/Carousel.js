...
const Carousel = props => {
  ...
  const changeCenter = ({ event, index }) => {
    ...
    if (index !== currFirstImg) {
      ...
      window.open(imgList[currentImgId]); // Can have a callback in props and execute it on click.
    }
  };
  ...
  const cycleToNextImage = actual => {
    if (
      ...
      currMiddleImgRef.current =
        currMiddleImgRef.current - 1 > -1 ? currMiddleImgRef.current - 1 : imgList.length - 1; // Right side image click
      setCurrFirstImg(currMiddleImgRef.current);
    } else {
      // Left side image click
      currMiddleImgRef.current =
        currMiddleImgRef.current + 1 < imgList.length ? currMiddleImgRef.current + 1 : 0; // Conditions to handle cycle
      setCurrFirstImg(currMiddleImgRef.current);
    }
  };
      ...
      >
        {imgList.map((url, index) => {
          ...
          return (
            ...
            >
              <img src={url} alt={"img_" + index} width={img_width} height={img_height} />
            </li>
          );
        })}
      </ul>
